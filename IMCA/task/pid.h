...
/*定义PID结构体初始化函数*/
void PID_struct_init(pid_t* pid, uint32_t mode, uint32_t maxout, uint32_t intergral_limit,float deadband, float kp, float ki, float kd);
/*PID计算函数*/
float pid_calc(pid_t* pid, float fdb, float ref);
void abs_limit(float *a, float ABS_MAX);
float pid_calc_stand(pid_t* pid, float get, float set,float gyro_speed);
