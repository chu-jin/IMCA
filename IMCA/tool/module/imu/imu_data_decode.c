...
/*  callback function of  when recv a data frame successfully回调函数，当recv数据帧成功 */
	...
	while(offset < pkt->payload_len)
	{
		switch(p[offset])
		{
            ...
            case kItemRotationEul://欧拉角
                ...
                stream2int16(temp, p + offset + 1);
                receive_imusol.eul[1] = (float)temp[0] / 100;//pitch轴俯仰角
                receive_imusol.eul[0] = (float)temp[1] / 100;//roll轴横滚角
                receive_imusol.eul[2] = (float)temp[2] / 10;//yaw轴横向角
                offset += 7;
                break;
